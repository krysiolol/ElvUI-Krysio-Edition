...
function E:DBConversions()
	...
	do -- Death Knight runes default
		-- The Player Classbar (Death Knight runes) now defaults to enabled and sits at the
		-- top of the Player unit frame. Force it on once for existing Death Knight profiles
		-- so the runes appear without manual setup. Guarded by an account-wide flag so the
		-- player can still disable it afterwards and have that choice respected.
		if not E.global.dkRuneClassbarApplied then
			E.db.unitframe.units.player.classbar.enable = true
			E.global.dkRuneClassbarApplied = true
		end
	end
