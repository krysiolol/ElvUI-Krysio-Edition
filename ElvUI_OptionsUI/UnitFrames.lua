...
local function GetRoleSortString(group)
	local order
	if group == "party" then
		order = E.db.unitframe.roleSortOrderParty or "TANK,HEALER,DAMAGER"
	else
		order = E.db.unitframe.roleSortOrderRaid or "TANK,HEALER,DAMAGER"
	end
	if order == "" then order = "TANK,HEALER,DAMAGER" end
	return order
end
...
E.Options.args.unitframe.args.player = {
	...
	args = {
		...
		classbar = {
			order = 750,
			type = "group",
			name = L["Death Knight Runes"] or "Death Knight Runes",
			...
			args = {
				...
				additionalPowerText = {
					...
					hidden = function() return true end, -- Grimfall: not used by rune bar
					disabled = function() return not E.db.unitframe.units.player.classbar.enable end
				},
				spacer = {
					order = 7,
