...
function MBG:GrabMinimapButtons()
	for _, frame in ipairs(self.minimapFrames) do
		-- Call GetChildren() once into a table instead of once per loop
		-- iteration. The original `select(i, frame:GetChildren())` pattern
		-- pushes every child onto the Lua C stack on each iteration, which
		-- the 3.3.5a client can't handle once the Minimap has accumulated
		-- many children from other addons (stack overflow). pcall guards
		-- the call itself in case the child list alone is too large.
